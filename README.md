...
The default scriptable object UI is extended to further provide utilities for reordering, sorting records & string sets, and record type enforcement. Supported record types include: booleans, ints, floats, strings, and string sets (unique strings joined by semicolons). Although ordering doesn't really matter when accessing key-value record pairs & string sets in scripts, it could be helpful for data management via the inspector.
