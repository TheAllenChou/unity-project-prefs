...
The default scriptable object UI is extended to further provide utilities for reordering, sorting, and record type enforcement. Although ordering doesn't really matter when accessing key-value pairs in scripts, it could be helpful for managing the records in the inspector. Supported record types include: booleans, ints, floats, strings, and string sets (unique strings joined by semicolons).
